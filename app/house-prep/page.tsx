...
export default function HousePrepPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true)
    const [loadedTasks, loadedExpenses] = await Promise.all([
      getTasks(hub.id),
      getExpenses(hub.id),
    ])
  ...
  const handleSaveExpense = async (expenseData: Omit<Expense, "id"> | Expense) => {
    ...
    if ("id" in expenseData && expenseData.id) {
      ...
      await dbUpdateExpense(expenseData.id, expenseData)
      setExpenses(expenses.map((e) => (e.id === expenseData.id ? { ...e, ...expenseData } : e)))
    } else {
      // Adding new expense
      const newExpense = await dbAddExpense(hub.id, expenseData as Omit<Expense, "id">)
      if (newExpense) {
